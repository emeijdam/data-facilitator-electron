import { makeStyles, tokens, Button ,  Tree, TreeItem, TreeItemLayout} from '@fluentui/react-components';
...
const DefaultTreeItemLayout: React.FC<treeProps> = () => {
  ...
  const listItems2 = state.workbenchItems.map((workBenchItem: any) =>
    <TreeItem itemType="leaf" key={workBenchItem.id} onClick={() => dispatch({ type: ActionType.SetActiveWorkbenchItem, payload: workBenchItem.id })} className={state.activeWorkBenchIndex === workBenchItem.id ? classes.isaan : classes.isuit}>
  ...
  return (
    <Tree aria-label="Tree" defaultOpenItems={['default-subtree-1']}>
      <TreeItem id="default-subtree-1" itemType="branch">
